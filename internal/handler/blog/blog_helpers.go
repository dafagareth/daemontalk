...
func (h *Handler) BlogPostsPartial(w http.ResponseWriter, r *http.Request) {
	...
	visible := h.getVisiblePosts(h.isAdmin(r))
	...
	total := len(filtered)
	totalPages := (total + common.DefaultPostsPerPage - 1) / common.DefaultPostsPerPage
	if totalPages < 1 {
		totalPages = 1
	}
	if page > totalPages {
	...
	start := (page - 1) * common.DefaultPostsPerPage
	end := start + common.DefaultPostsPerPage
	if end > total {
		end = total
	}
	...
	h.render(w, r, blogtmpl.BlogPostItems(ui, pagePosts, lang, viewCounts, page+1, hasMore, tagFilter))
}
