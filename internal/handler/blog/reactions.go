...
func (h *Handler) PostReaction(w http.ResponseWriter, r *http.Request) {
	...
	title := ""
	date := ""
	if p, ok := post.FindBySlug(h.getAllPosts(), slug); ok {
		title = p.Title
		date = p.Date.Format("2006-01-02")
	}

	cookieName := common.CookieReactedPrefix + slug
	if cookie, err := r.Cookie(cookieName); err == nil && cookie.Value != "" {
		...
		if oldEmoji == emoji {
			...
			h.render(w, r, blogtmpl.ReactionsBar(ui, reactions, slug, lang, "", commentCount, title, date))
			return
		}
		...
		http.SetCookie(w, &http.Cookie{
			...
			Path:     "/",
			MaxAge:   common.CookieReactionMaxAge,
			HttpOnly: true,
			SameSite: http.SameSiteLaxMode,
		})
		h.render(w, r, blogtmpl.ReactionsBar(ui, reactions, slug, lang, emoji, commentCount, title, date))
		return
	}
